...
* @return {boolean}
*/
function isNamcoDoctor(provider = {}) {
...
*/
function getPrescriptionSourceEncounterUuids(visit = {}) {
...
class LibPresciptionComponent {
    ...
    recommendation;
    brandName = false;
    isNamcoDoctorLoggedIn = isNamcoDoctor(getCacheData(true, doctorDetails.PROVIDER));
                ...
                this.visitService.patientInfo(this.baseUrl, visit.patient.uuid).subscribe((patient) => {
                    if (patient) {
                        ...
                        if (!this.isNamcoDoctorLoggedIn) {
                            const referringVisitNote = (visit.encounters || []).find((e) => (e?.display || '').includes(visitTypes.VISIT_NOTE) && !(e?.display || '').includes(visitTypes.SPECIALIST_VISIT_NOTE));
    ...
    */
    buildConsultedDoctorFromProvider(provider) {
        const d = {
   ...
   */
    ...
    */
    ...
    isFeatureAvailable(featureName, notInclude = false) {
        if ((featureName === 'followUpType' || featureName === 'followUpTime') && this.isNamcoDoctorLoggedIn)
            return !notInclude;
